...
def init_sources():
    """Build the internal source representation used for source selection."""
    global n_sources, x_source, y_source, x_source_end, y_source_end
    ...
    if n_sources < 1:
        raise ValueError("the number of source entities must be at least 1")

    x_source = np.zeros(n_sources, dtype=float)
    y_source = np.zeros(n_sources, dtype=float)
    x_source_end = np.zeros(n_sources, dtype=float)
    y_source_end = np.zeros(n_sources, dtype=float)

    if vent_flag in (0, 1, 8):
        x_source[:] = x_vent[:n_sources]
        y_source[:] = y_vent[:n_sources]
        x_source_end[:] = x_vent[:n_sources]
        y_source_end[:] = y_vent[:n_sources]

    elif vent_flag in (2, 3, 6):
        x_source[:] = x_vent[:-1]
        y_source[:] = y_vent[:-1]
        x_source_end[:] = x_vent[1:]
        y_source_end[:] = y_vent[1:]
        ...
        x_source[:] = x_vent[:n_sources]
        y_source[:] = y_vent[:n_sources]
        x_source_end[:] = x_vent_end[:n_sources]
...
def build_source_cdf():
    ...
    if vent_flag in (2, 4):
        ...
        if np.sum(prob) <= 0.0:
            raise ValueError("source_probabilities must have a positive sum")
        cum_source_prob[:] = np.cumsum(prob)
        cum_source_prob[:] = cum_source_prob[:] / cum_source_prob[-1]

    cum_source_prob[-1] = 1.0

    if np.any(cum_source_prob[1:] < cum_source_prob[:-1]):
        raise ValueError("cum_source_prob is not monotonic")
...
def prepare_source_activation_arrays(total_volume):
    ...
    if source_volume_from is None:
        source_volume_from = np.zeros(n_sources, dtype=float)
    else:
        source_volume_from = np.asarray(source_volume_from, dtype=float)
        if source_volume_from.ndim == 0:
    ...
    if source_volume_to is None:
        source_volume_to = np.full(n_sources, default_to, dtype=float)
    else:
        source_volume_to = np.asarray(source_volume_to, dtype=float)
        if source_volume_to.ndim == 0:
...
def first_lobe(flow, Zflow):
    ...
    if vent_flag in (0, 1, 3, 5):
        active_source_weight[source_is_active] = 1.0

    elif vent_flag in (2, 4):
        active_source_weight[0] = cum_source_prob[0]
        active_source_weight[1:] = cum_source_prob[1:] - cum_source_prob[:-1]
    ...
    if vent_flag == 0:
        ...
        i_source = int(np.floor(flow * len(active_idx) / n_flows))
        i_source = min(i_source, len(active_idx) - 1)
...
init_sources()
build_source_cdf()
prepare_source_activation_arrays(total_volume)

print('Number of vents', n_vents)
print('Number of discrete sources', n_sources)
print('')
