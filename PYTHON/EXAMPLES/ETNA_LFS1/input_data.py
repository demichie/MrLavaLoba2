...
# this determines how large an area will be clipped from the DEM.
# smaller = faster, but need to ensure nothing runs out of the domain
east_to_vent = 5000.0
west_to_vent = 5000.0
...
# Total volume (this value is used only when volume_flag = 1) set "1" to be
# confirmed.
...
# Area of each lobe ( only effective when volume_flag = 0 or
# fixed_dimension_flag = 1 )
lobe_area = 1000   # m^2

# Thickness of each lobe ( only effective when volume_flag = 0 or
# fixed_dimension_flag  2 )
# avg_lobe_thickness = 0.07  # m

# Ratio between the thickness of the first lobe of the flow and the thickness
# of the last lobe.
...
# computation of the slope, but the thickness is still correct. this allows
# for "channel" flow, if = 1, then sublava flow would not happen.
...
# Inertial exponent:
# inertial_exponent = 0 => the max probability direction for the new lobe is
#                          the max slope direction;
# inertial_exponent > 0 => the max probability direction for the new lobe takes
#                          into account also the direction of the parent lobe
#                          and the inertia increaes with increasing exponent
